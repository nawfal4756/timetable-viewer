...
export default function Selection() {
  ...
  const [coursesList, setCoursesList] =
    useState<SubjectSection[]>(subjectSectionCombo);
  const [accordionSelected, setAccordionSelected] = useState<string | false>(
    "selection"
  );
  ...
  useEffect(() => {
    ...
    removeAlreadyAdded = searchFiltered.filter((course) => {
      return !courses.some((course2) => {
        return course.id == course2.id;
      });
    });
  ...
  const handleSaveCourses = () => {
    ...
    localStorage.setItem(
      "courses",
      JSON.stringify(courses.concat(tempCourses))
    );
    ...
    <div>
      ...
      <div>
        <Grid container sx={{ paddingTop: "2%" }}>
          ...
          <Grid
            xs={12}
            sx={{ paddingY: "1%", display: "flex", justifyContent: "flex-end" }}
          >
            <Link href="/">
          ...
          <Grid xs={12}>
            <Accordion
              expanded={accordionSelected === "selection"}
              onChange={handleAccordionChange("selection")}
            >
              <AccordionSummary expandIcon={<ExpandMore />}>
                <Typography>Course Selecion</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Grid container>
                  <Grid
            ...
            <Accordion
              expanded={accordionSelected === "selected"}
              onChange={handleAccordionChange("selected")}
            >
              <AccordionSummary expandIcon={<ExpandMore />}>
                <Typography>Course Selecion</Typography>
              </AccordionSummary>
                          ...
                          {courses.map((course) => {
                            return (
                              <TableRow key={course.id}>
                                ...
                                <TableCell>
                                  <Typography>{course.teacher}</Typography>
                                </TableCell>
                                <TableCell>
                                  <Button
                                    variant="contained"
